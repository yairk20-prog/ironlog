...
1. עמוד "סטורי" יומי בדף הבית שמציג תצוגה מקדימה של אימון המחר (הוצע, טרם נבנה).
2. להגדיר `ANTHROPIC_API_KEY` ב-Netlify כדי שהמאמן יעבוד בפרודקשן.
3. זיהוי קלוריות מתמונה — התשתית קיימת בפונקציית ה-coach, צריך מסך צילום.
