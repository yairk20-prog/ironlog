...
export const EQUIPMENT = [
  { id: 'FreeWeights', emoji: '🏋️', name: 'משקולות חופשיות', desc: 'מוט, משקולות יד, ספסל' },
  { id: 'Machine', emoji: '⚙️', name: 'מכונות', desc: 'לחיצות, פולי מכני, האק' },
  { id: 'Cable', emoji: '🪢', name: 'כבלים / פולי', desc: 'פולי עליון ותחתון, קרוסאובר' },
  { id: 'Bodyweight', emoji: '🤸', name: 'משקל גוף', desc: 'מתח, מקבילים, שכיבות' },
  { id: 'Bands', emoji: '🎗️', name: 'רצועות התנגדות', desc: 'גומיות אימון' }
];

export const LIMITS = [
  { id: 'shoulder', emoji: '💪', name: 'כתף רגישה', desc: 'נדלג על לחיצות מעל הראש כבדות' },
  { id: 'knee', emoji: '🦵', name: 'ברך רגישה', desc: 'נעדיף לחיצת רגליים על סקוואט עמוק' },
  { id: 'back', emoji: '🦴', name: 'גב תחתון רגיש', desc: 'נדלג על דדליפט וחתירה בכפיפה' },
  { id: 'wrist', emoji: '✋', name: 'שורש כף יד', desc: 'נעדיף אחיזה ניטרלית ומכונות' }
];

const EXPERIENCE = [
  { id: 'beginner', emoji: '🌱', name: 'מתחיל', desc: 'פחות משנה של אימוני משקולות — נתחיל בנפח מתון' },
  { id: 'intermediate', emoji: '🔥', name: 'בינוני', desc: '1–3 שנים — נפח מלא והעלאות שבועיות' },
  { id: 'advanced', emoji: '⚡', name: 'מתקדם', desc: '3+ שנים — נפח גבוה ומעקב RIR מדויק' }
];

const DAYS = [
  { id: 'ppl3', emoji: '3️⃣', name: '3 ימים בשבוע', desc: 'אימון יום־כן־יום, מתאים ללו״ז עמוס' },
  { id: 'ppl5', emoji: '5️⃣', name: '5 ימים + יציבה', desc: 'סבב PPL עם יום יציבה ושתי מנוחות' },
  { id: 'ppl6', emoji: '6️⃣', name: '6 ימים בשבוע', desc: 'כל קבוצת שרירים פעמיים — הכי מהיר' }
];

const SEX = [
  { id: 'male', emoji: '👨', name: 'גבר' },
  { id: 'female', emoji: '👩', name: 'אישה' },
  { id: 'other', emoji: '🧑', name: 'מעדיף לא לציין' }
];
...
function stepWelcome(a, repaint) {
  const box = el('div', {}, [
    el('div', { style: { fontSize: 'clamp(46px,14vw,66px)', lineHeight: '1', marginBottom: '6px' }, text: '🏋️' }),
...
function stepGoal(a, repaint) {
  return el('div', {}, [
    ...
    ...Object.values(GOALS).map((g) => pick(
      { emoji: g.id === 'hypertrophy' ? '💪' : g.id === 'strength' ? '🏆' : g.id === 'cut' ? '🔥' : '🎯', name: g.name, desc: g.desc },
      a.goal === g.id,
      () => { a.goal = g.id; repaint(); }
    ))
  ]);
...
function stepLimits(a, repaint) {
  ...
  return el('div', {}, [
    ...
    pick(
      { emoji: '🧍', name: 'מעניין אותי לשפר יציבה', desc: 'נוסיף יום יציבה ותרגילים מתקנים לאגן ולגב העליון' },
      a.posture,
...
function stepDone(a) {
  ...
  return el('div', {}, [
    el('div', { style: { fontSize: 'clamp(46px,14vw,66px)', lineHeight: '1', marginBottom: '6px' }, text: '✅' }),
...
function pick(o, on, onclick) {
  return el('button', { class: `pick${on ? ' on' : ''}`, onclick }, [
    el('div', { class: 'pick-ico', text: o.emoji }),
    el('div', { class: 'grow' }, [
      el('b', { text: o.name }),
